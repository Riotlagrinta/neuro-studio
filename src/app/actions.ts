...
export async function generateContent(topic: string): Promise<ContentPlan> {
  ...
  const systemPrompt = `
    ...
    2. VISUELS (visualPrompt) : Génère des prompts en ANGLAIS descriptifs mais concis (max 50 mots). Utilise des termes techniques : "cinematic lighting", "8k", "hyper-realistic".
    ...
    Réponds UNIQUEMENT avec un objet JSON structuré comme suit :
    {
      ...
      "category": "La niche exacte",
      "scenes": [
        {
          "id": 1,
          "voiceOver": "Le texte parlé...",
          "visualPrompt": "Concise English prompt for AI image...",
  ...
  try {
    ...
    if (!content) {
      ...
      throw new Error("AUCUN_CONTENU_IA");
    }
    ...
    try {
      ...
      if (!parsed.scenes || !Array.isArray(parsed.scenes)) {
        throw new Error("FORMAT_SCENES_INVALIDE");
      }
      ...
      throw new Error("ERREUR_JSON_IA");
    }
  } catch (error: any) {
    console.error("Erreur critique generateContent:", error);
    throw new Error(error.message || "ECHEC_IA");
  }
}
...
export async function getElevenLabsAudio(text: string) {
  try {
    ...
    if (!apiKey) throw new Error("API_KEY_MANQUANTE");
    ...
    console.error("Erreur Audio Action:", error.message);
    throw new Error(error.message || "ECHEC_AUDIO");
  }
}
